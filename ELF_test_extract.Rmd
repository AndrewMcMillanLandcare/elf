...
# Now intersect the CLTL_adj with LCDB
...
# Get diff between clipped features and LCDB features that intersect with CLTL
...
# set DEM zonal stats of treeline layer
...
names(LCDBxCLTL_adj_w_ZSTATS)

```


# Now apply the rules set out in Section 3.2ii of Caitlin's Report
...
New_TL_unfilt_w_FLAGS = outside_polys_w_ZSTATS_mdf %>% 
  ...
  mutate(
    ...
    KEEP_FLAG = case_when(
      ...
      (Class_2018 == 16) & (ORIG_AREA_ha < 10)  ~ "KEEP",
...
New_TL_unfilt_w_FLAGS_ffn = paste0(AL_dn,"New-Treeline-Layers/New_TL_unfilt_w_FLAGS.gpkg")
st_write(New_TL_unfilt_w_FLAGS, New_TL_unfilt_w_FLAGS_ffn, append = F)
