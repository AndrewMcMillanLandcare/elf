...
# outside_polys_filtered_buff = st_buffer(outside_polys_filtered, 10)
outside_polys_filtered_UNN = outside_polys_filtered %>% 
  validate_sh() %>% 
  group_by(LCDB_UID) %>% 
...
all_polys_filtered_UNN = st_union(outside_polys_filtered_UNN, inside_polys_filtered_UNN, by_feature = T)


LCDBxCLTL_adj %>% pull(Class_2018) %>% unique()
