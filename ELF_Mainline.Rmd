...
rdn = "T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/"


source(paste0(rdn,"AndrewsWork/MyCode/R/elf/ea_utils.R"))
source(paste0(rdn,"AndrewsWork/MyCode/R/elf/ELF_utils.R"))
source("C:/Users/McMillanAn/OneDrive - MWLR/Code/R/utils/mcm_sp_utils.R")
#set up the data folders
# rdn = "K:/rpas/ELF/ELF_Project/"
...
CRS = st_crs(SMAP)
FSL = st_transform(FSL_NZGD49, CRS)

SMAP_cmb = st_combine(SMAP)

datadir = "C:/Users/McMillanAn/OneDrive - MWLR/Projects/PRJ3597-MPI-ELF/data/"

dir.exists(datadir)

FSL_MINUS_SMAP = ea_intersection(FSL,SMAP_cmb,  PC = Current_PC, "FSL_MINUS_SMAP", dn = datadir, val = T, parallel = T, diff_opt = T, ncore = 16, filetype = "SHP" )
...
LCDB5_ffn = "K:/rpas/ELF/ELF_Project/ELFData/LCDB/lcdb-v50-land-cover-database-version-50-mainland-new-zealand.gpkg"
...
dem_ffn = "K:/rpas/ELF/ELF_Project/ELFData/NZ_DEM/dem_linz_nz_110520_orthonzgd49h_nztm_10m.kea"
dem = rast(dem_ffn)
...
improved_treeline_lines_ffn = paste0(NTL_dn, "improved_treeline_lines.gpkg")
st_write(ITL, improved_treeline_lines_ffn, append = F)


mapview(PL)

samp_points = st_intersection(ITL, PL)
...
## ML - 1. create the array of sampling points by intersecting the LCDB Treeline with an array of horizontal lines . Then intersect that layer with the available Lidar layer

```{r}

#get the LCDB tree line

LCDB_treeline_ffn = paste0(NTL_dn, "improved_treeline_lines.gpkg")
LCDB_treeline = st_read(LCDB_treeline_ffn)


mapview(LCDB_treeline)


#get the parallel lines

PL = create_parallel_lines(x_interv=2500, y_interv = 2500)

PLxNTL = st_intersection(PL, LCDB_treeline) 
PLxNTL_points = PLxNTL %>% st_cast("POINT")

mapview(PLxNTL_points)


# Now find the points with LIDAR

lidar_tiles = st_read(paste0("K:/rpas/ELF/ELF_Project/AndrewsWork/Lidar-Index-Tiles-LINZ/All_LINZ_LiDAR_Index_Tiles.shp"))


# intersect the Sampling points with the avaialble Lidar Layers

PLxNTLxlidar = st_intersection(PLxNTL_points, lidar_tiles)
mapview(PLxNTLxlidar)



PLxNTLxlidar_ffn = paste0(NTL_dn, "PLxNTLxlidar.geojson")
st_write(PLxNTLxlidar, PLxNTLxlidar_ffn, append = F)
...
## ML - 2. Read the PLxNTLxlidar file and select sampling points

```{r}

PLxNTLxlidar_ffn = paste0(NTL_dn, "PLxNTLxlidar.geojson")
SP_array = st_read(PLxNTLxlidar_ffn)

SP_array %>% pull(NAME) %>% unique()
...
## ML - 3. Find the Correct DTM for the Point
...
Lidar_Survey = SP_sf$NAME
Lidar_tilename = SP_sf$tilename





lidar_all = st_read("K:/rpas/ELF/ELF_Project/AndrewsWork/Lidar-Index-Tiles-LINZ/All_LINZ_LiDAR_Index_Tiles.shp")


lidar_all_nogeo = lidar_all %>% st_set_geometry(NULL)
lidar_all_nogeo %>% filter(is.na(tilename )) 



lidar_all_nogeo %>% filter(!is.na(tilename )) %>% nrow()
lidar_all_nogeo %>% filter(tilename == "<NA>") %>% nrow()

lidar_all_nogeo %>% pull(tilename) %>% first()



DTM_ffn = paste0("K:/rpas/ELF/ELF_Project/marlborough/marlboroughData/marlborough-2020-2021-DEM.kea")

DTM = read_stars(DTM_ffn)
DTM_10m = st_as_stars(st_bbox(DTM), dx = 10)
DTM_10m = st_warp(DTM, DTM_10m)


DTM_degr = st_warp(DTM, cellsize = 10, use_gdal = T)


write_stars(DTM, "K:/rpas/ELF/ELF_Project/marlborough/marlboroughData/marlborough-2020-2021-DEM_10m2.kea")

DTM_poly = st_as_sf(DTM_degr)

DTM_outline = as.polygons(DTM_degr, trunc=T, dissolve = T)
...
## ML - 4. Find the Steepest Descent
...
PHI_st = find_steepest_angle(LEN, WID, DIV_deg, SP)
...
ELF_2012_ffn  ="C:/Users/McMillanAn/OneDrive - MWLR/NZ-Geospatial-Data/mfe-environmental-limiting-factors-2012-SHP/environmental-limiting-factors-2012.shp"
...
UM = ELF_2012 %>% filter(ULTRAMAFIC==1)# & LIMITATION == "Multiple")
mapview(UM)
...
# Extracting the RAINFALL throu soil types

```{r}

RAIN_ffn = "T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/DATA/nz_mar/w001001.adf"

RAIN = rast(RAIN_ffn)
plot(RAIN)

SMAP_FSL_CMB_NZSC_ffn = "T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/DATA/SOIL_Limitation_layers/SMAP_FSL_CMB_NZSC.shp"

SMAP_FSL_CMB_NZSC = st_read(SMAP_FSL_CMB_NZSC_ffn)

moisture_limiting_soils_list = c("S","WX", "RXA", "RXT")

LIMITING_SOILS = SMAP_FSL_CMB_NZSC %>% 
  dplyr::mutate(POLY_UID = 1:nrow(.)) %>% 
  filter(NZSC %in% moisture_limiting_soils_list) 
...
mapview(LIMITING_SOILS)


library(exactextractr)

LIMITING_SOILS_w_RAIN = exact_extract(SMAP_FSL_CMB_NZSC, LIMITING_SOILS, )
...
WONI_ffn = "T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/DATA/Wetlands.gdb"

WONI = st_read(WONI_ffn, layer = "Current_wetland_sites")

names(WONI)
WONI %>% pull(Source) %>% unique()

WONI_PAK = WONI %>% filter(str_detect(Source,"pak") | str_detect(Source,"Pak"))
WONI_PAK %>% pull(Source) %>% unique()

SI_WL = rast("T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/DATA/WONI/2015-01-22MfE/sihisto_typo.img")

SI_WL_sel = SI_WL == 1

NI_WL = rast("T:/Palmerston North/Projects A-E/Environmental Limiting Factors/ELF_Project/DATA/WONI/2015-01-22MfE/nihisto_typo.img")
...
SI_WL_num = as.numeric(SI_WL)


SI_WL_PAK = SI_WL[SI_WL==1]
plot(SI_WL_PAK)

SI_WL_r=raster(SI_WL)

SI_WL_val = as.numeric(SI_WL,1) 
SI_WL_val = as.numeric(SI_WL,2)

SI_WL_val_PAK= SI_WL_val
SI_WL_val_PAK[SI_WL_val_PAK!=1]=0

SI_WL_derat = deratify(SI_WL_r)
...
PAK_vals = SI_WL_r@data@attributes
...
# DEV 1 - accessing Lidar data through the LINZ API

```{r}


my_WMTS_API_key = "f1d3cc0f02134a549b8e1071b48e2639"



layer_url = paste0('https://data.linz.govt.nz/services;key=',my_WMTS_API_key,'/wmts/1.0.0/layer/53592/WMTSCapabilities.xml')



bbox = QgsRectangle(19456867.78889, -5061757.6665, 19462438.3959, -5059324.4087)



library(httr)

REQUEST = "https://data.linz.govt.nz/services;key=f1d3cc0f02134a549b8e1071b48e2639/wmts/1.0.0/layer/53621/WMTSCapabilities.xml"


res = GET(REQUEST)

https://tiles-cdn.koordinates.com/services;key=f1d3cc0f02134a549b8e1071b48e2639/tiles/v4/layer=53621/EPSG:3857/{z}/{x}/{y}.png
...
Lidar_Index_Tiles = "K:/rpas/ELF/ELF_Project/AndrewsWork/Lidar-Index-Tiles-LINZ/All_LINZ_LiDAR_Index_Tiles.shp"
...
filetbl = read_csv("K:/rpas/ELF/ELF_Project/AndrewsWork/Lidar-Data-File-Table.csv")
...
names_in_tile_index = c("wellington", "not-present", 
                        ...
                        dn = "K:/rpas/ELF/ELF_Project/Taranaki/TaranakiProfiles/"
                        fn = "taranaki2_chmProfile.json"
                        ffn = paste0(dn,fn)
                        
                        dn = "K:/rpas/ELF/ELF_Project/gw/gwProfiles/"
                        ...
                        coords_shp_ffn =  "K:/rpas/ELF/ELF_Project/AndrewsWork/TestTr.shp"
                        ...
                        source("K:/rpas/ELF/ELF_Project/AndrewsWork/MyCode/R/ea_utils.R")
                        
                        LCDB5_ffn = "K:/rpas/ELF/ELF_Project/ELFData/LCDB/lcdb-v50-land-cover-database-version-50-mainland-new-zealand.gpkg"
                        file.exists(LCDB5_ffn)
                        
                        # dem_dn = "K:/rpas/ELF/ELF_Project/ELFData/NZ_DEM/"
                        ...
                        root1 = "K:/rpas/ELF/ELF_Project/AndrewsWork/"
                        ...
                        DIROUT = "K:/rpas/ELF/ELF_Project/AndrewsWork/"
                        ...
                        dn = "K:/rpas/ELF/ELF_Project/AndrewsWork/Lidar-Index-Tiles-LINZ/"
